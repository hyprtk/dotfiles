...
# Install pamac GUI only where it is known to work (Manjaro, Arch, ArcoLinux)
case "$(detect_distro)" in
    manjaro|arch)
        _installOrUpdateYay pamac-all
        _installOrUpdateYay libpamac-full
        _installOrUpdateYay pamac-cli
        ;;
    *)
        echo "Skipping pamac packages (not supported on this distro)"
        ;;
esac
